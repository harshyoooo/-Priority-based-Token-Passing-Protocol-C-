...
void sender(Packet &pack, device &b, device &s, int expected, int gen, int last) {
    ...
    int gen_seq1 = gen + expected;  // starting sequence number
    vector<frames> f;
    int gen_seq = gen_seq1;

    for (int i = 1; i <= min(30, last - gen_seq1 + 1); i++) {
        int a = gen_seq - gen;
        if (a >= s.data_to_be_sent[b.id].size()) break;

        if (b.check[s.id][gen_seq % N] == 0) {
    ...
    if (receiver(f, b, s, last, d, got, first) == 0) {
        if (pack.retry_count >= 1) {
        cout << "[Retrying] Sender " << s.id << " retransmitting missing frames to Receiver " << b.id 
             << ", retry count = " << pack.retry_count + 1 << endl;
    }
        pack.arrival_time = curr_time.load();
        pack.sequence_no = gen_seq1 + d.front();  // retry from missing
        pack.no_packets -= got;
        pack.retry_count++;
        Multilevel_queue_push(pack);
    } else {
...
//         dev++;
//     }
//     int total=words.size()*MAX_DEVICES*(MAX_DEVICES-1);
//      vector<thread> device_threads;
//     for (int i = 0; i < MAX_DEVICES; ++i) {
//         device_threads.emplace_back(device_thread, i);
//     }
...
//     // Small delay to allow re-queued packets
//     this_thread::sleep_for(chrono::milliseconds(10));
